...
`python3 tests/ascii_field.py` runs the onboarding terrain's math (`ui/AsciiField.js`) through Qt's `qml6` tool against the reference wallet's golden vectors: terrain brightness and levels, the currency hash, the vault door, the morph lerp, the erosion curve, and the lens envelopes.

The native worker test injects test-only controls into a temporary QML copy and starts a separate fake Omarchy shell exposing lock state. It walks the actual onboarding: one step into restore and back, Create Wallet with no password, revealing and acknowledging the seed phrase, adding the fake mint from the first-mint step, and the handoff into the wallet. It then checks automatic reopening after desktop lock while hidden, invoice QR display, automatic receipt while hidden, send review/confirmation, enabling the password through Security controls, phrase display, clearing sensitive views on lock, and password unlock with the right balance. It never locks the actual desktop. Allow roughly 45 seconds for the background timer. `CASHU_ME_TEST_CAPTURE` needs `CASHU_ME_WINDOW=1` to have a window to grab, and then also exports the welcome, restore, seed and first-mint steps.
