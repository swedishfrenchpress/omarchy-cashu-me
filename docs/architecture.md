...
Closing hides the window. Desktop locking is polled through Omarchy's lock IPC every second, with a two-second timeout. An unavailable shell is treated as locked. Locking stops the wallet worker and clears sensitive UI views. This is bounded polling, not an instantaneous lock notification. Unlock starts a fresh worker and requires the password again. There is no persistent unlocked background daemon after Quit.
