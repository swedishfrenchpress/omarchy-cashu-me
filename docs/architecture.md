...
Every user-facing error is a fixed string, which alone leaves no way to separate a structural mint incompatibility from a transient network failure. Setting `CASHU_ME_LOG` to a file path turns on error diagnostics: the worker appends the operation that failed and the underlying library's message, bounded per line and created 0600. No password, recovery phrase, token, or request body is ever recorded, but a mint or database error can quote protocol detail, so treat the file as sensitive. Diagnostics are off unless that variable is set, and the interface never forwards worker stderr to the QML log.
