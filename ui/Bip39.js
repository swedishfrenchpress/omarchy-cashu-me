...
// The BIP-39 English wordlist (2048 words, the bip39 crate's copy), so the
// restore step can check each word as it is typed and offer completions,
// as cashubtc/wallet's word-by-word seed entry does. The checksum still runs
// in the worker; this list only says whether a word exists.
var words = [
...
function contains(word) { return lookup[word] === true }

// Up to `limit` words starting with `prefix`; none for an empty prefix.
function completions(prefix, limit) {
    var out = []
    if (!prefix) return out
    for (var j = 0; j < words.length && out.length < (limit || 3); j++) if (words[j].indexOf(prefix) === 0) out.push(words[j])
    return out
}
