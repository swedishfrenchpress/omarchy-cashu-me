...
- **2026-09-11:** User asked for cashubtc/wallet's onboarding, ASCII animations included, in native Omarchy UI, and chose after questioning: the full create path (seed card with acknowledgement, then first mint), the vault door on the seed-entry step since desktop has no iCloud chooser, every motion piece (terrain, vault morph, pointer lens, entrance fade, handoff curtain), the single-field seed entry kept, "Private cash. On your desktop.", no persisted onboarding state (an interrupted onboarding opens the wallet), the What-is-ecash page, and the unlock page on the same chassis. Ported the field's math verbatim with a parity test against the reference's vectors; the renderer is a sprite-sheet Canvas at 30 fps. The old three-ring Welcome is gone.
- **2026-09-11 (later):** User asked to follow cashubtc/wallet's conventions for adding mints and for recovery, reversing the single-field seed entry chosen earlier. Add mint from the Mints tab is now the bare URL form; the home and payment empty states open the connect picker with Known mints and Add by URL; restore enters words one at a time with the rail, card, completions and checksum review grid.
- **2026-09-11 (evening):** Empty states drop the 56px Nerd Font glyph (drawn for bar sizes, it scaled into a floating hairline) for the glyph at its bar size inside a bordered square, the shape the header icons and mint avatars already use; then, since the bar-size glyph still looked wrong, custom ASCII icons, first box-drawing line icons and then terrain-style sprites; the user rejected each, and text-only too ("still no illustrations?"). What finally reads is shaded ASCII art: a lit shape sampled onto a 26 × 13 grid and mapped onto a density ramp (`tools/ascii-art.py` → `ui/AsciiArt.js`; coin with ₿, clock, magnifier, funnel). A frozen patch of the real terrain was mocked too and looked like dim noise at that size. Same day: the QR page lost its Done button (Copy alone, back arrow exits) and History lost the "Showing up to 100 recent payments" footer. The faint dot pattern the user saw behind pages is the wallpaper through Omarchy's default window opacity (0.985 / 0.96 in `default/hypr/windows.lua`), not the wallet drawing anything. History's filter now matches the reference: All / Pending / Completed by status, replacing the direction tabs. The native test's post-onboarding captures had been stale copies since `show` switched to the compact panel; captures now expand to window mode and wait for a fresh file. Camera scan did nothing on the Mac: zbarcam defaults to `/dev/video0`, which is the Apple ISP's non-capture node, and exited at once; `bin/scan-qr` now picks the first V4L2 capture device by VIDIOC_QUERYCAP, with `CASHU_ME_CAMERA` as an override. Decoding through the real camera is still unverified by hand. Later: Check Status on Pending Ecash now shows only with "Check sent ecash" off, as both reference apps gate it (`!settings.checkSentTokens`), and an ecash send skips the review page, amount → token, as both reference apps do; the worker still prepares then confirms, the UI confirms the review the instant it arrives. The native test was rewritten for this (review persistence now uses a receive of the wallet's own token) but could not be run: Rust and the fake mint were gone from the machine.
